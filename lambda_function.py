...
def lambda_handler(event, context):
    min = int(event['queryStringParameters']['min']) if 'min' in event['queryStringParameters'] else 0
    max = int(event['queryStringParameters']['max']) if 'max' in event['queryStringParameters'] else 99
    response = requests.get(url = URL, params = { 'uuid': event['queryStringParameters']['uuid'] })
    result = []
